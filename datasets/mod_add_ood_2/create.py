...
* train set: $a\in [0,39]\cup[61,112], \quad b\in[0,69]\cup[91,112]$
* test set: $a\in [40,60], \quad b\in[70,90]$
'''
...
if __name__ == "__main__":
    p = 113
    train_a_range = np.concatenate((np.arange(0,40), np.arange(61,113)),axis=0)
    train_b_range = np.concatenate((np.arange(0,70), np.arange(91,113)),axis=0)
    train_samples = generate_dataset(train_a_range, train_b_range, p)
    test_a_range = np.arange(40,61)
    test_b_range = np.arange(70,91)
