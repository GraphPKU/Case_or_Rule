...
if __name__ == "__main__":
    ...
    dataset = generate_dataset(np.arange(0,a_range), np.arange(0,b_range))
    centers, lengths = generate_test(np.arange(0,a_range), np.arange(0,b_range), np.arange(5,15), 3)
    # centers = []
    # lengths = []
    title = f"a{a_range}_b{b_range}_{len(centers)}hole"


    test_samples = []
    train_samples = []
    img = np.zeros((a_range, b_range))
