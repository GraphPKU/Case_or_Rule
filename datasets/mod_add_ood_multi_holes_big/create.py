...
if __name__ == "__main__":
    ...
    a_range = p
    b_range = p
    number = f"p={p}_arange={a_range}"
    dataset = generate_dataset(np.arange(0, a_range), np.arange(0, b_range), p)
    centers, lengths = generate_test(np.arange(0, a_range), np.arange(0, b_range), [30], 1)
    # centers = [(108, 117)]
    # lengths = [20]
